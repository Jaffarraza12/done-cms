...
class MContent extends CI_Model {
        ...
        if ($this->db->insert('content', $arr))
        {
            ...
            foreach ($data['Languages'] as $lang)
            {
                ...
                $arr = array(
                    ...
                    'pdf_ids' => ($data['pdf_ids_' . $lang->code]),
                    'pdf_ids2' => ($data['pdf_ids2_' . $lang->code]),
                    'year' => ($data['year_' . $lang->code])
                );
    ...
    function edit($data)
    {
        ...
        foreach ($data['Languages'] as $lang)
        {
            ...
            // if (isset($data['image_' . $code])) {
            //     $arr['image'] = $this->MUtils->doUploadPath('image_' . $code, 1024, 768, false, 'pages');
            // }
    ...
    function view()
    {
        $q = "SELECT c.contentid cid, c.status, c.tag, cd.cdid cdid, cd.contentid content_id, cd.languageid languageid, cd.title, l.code, c.pdf, c.show_image, c.show_slider, c.show_recent_news, cd.pdf_ids, cd.pdf_ids2, cd.year FROM content c ";
